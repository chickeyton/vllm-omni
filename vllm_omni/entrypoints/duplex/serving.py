...
from vllm_omni.engine.duplex.commands import DuplexCommand, DuplexCommandError, SignalTurn
...
_UNJOURNALED_EVENTS = (SessionCreated, SessionResumed)
...
class _Attachment:
    ...
    generation: int
    resumed: bool = False
    resume_credential_delivered: bool = False
...
class OmniDuplexSessionHandler:
    ...
    async def _open(
        ...
        try:
            handle = await self._omni.open_session(session_payload, session_id=session_id)
        ...
        attachment = _Attachment(
            ...
            resume_credential_delivered=resume_supported,
        )
    ...
    async def _resume(
        ...
        if request is None:
            await send_json(
                envelope.error_payload(
                    ...
                    "session.resume requires session_id, incarnation, resume_token, and a non-negative event sequence",
                )
            )
            return None
        ...
        return _Attachment(
            handle=handle,
            generation=resumed.attachment_generation,
            resumed=True,
        ...
        while not handle.closed:
            ...
            if raw is None:
                with suppress(DuplexSessionError):
                    await handle.close(reason="timeout")
                return
    ...
    async def _on_disconnect(self, attachment: _Attachment) -> None:
        ...
        if handle.closed:
            return
        resumable = handle.capabilities.supports_session_resume and attachment.resume_credential_delivered
        is_current = resumable and await self._attachment_registry.is_current_attachment(
            handle.session_id, attachment.generation
        )
        if is_current and await self._attachment_registry.detach(
