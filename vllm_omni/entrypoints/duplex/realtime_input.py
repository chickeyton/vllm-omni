...
class RealtimeHandshake:
    ...
    pending_command_payload: dict[str, Any] | None = None
...
def parse_resume_request(event: Mapping[str, Any]) -> ResumeRequest | None:
...
class RealtimeEnvelope:
    ...
    @classmethod
    def from_query_params(cls, query_params: Any) -> RealtimeEnvelope:
        ...
        session_id = getter("session_id") if getter is not None else None
        return cls(
            default_model=model if isinstance(model, str) and model else None,
            default_session_id=session_id if isinstance(session_id, str) and session_id else None,
            resume_only=(
    ...
    def default_session_payload(self) -> dict[str, Any]:
        return {"model": self.default_model, "session_id": self.default_session_id}

    def initial_open_payload(self) -> dict[str, Any] | None:
    ...
    def first_message(self, payload: Mapping[str, Any]) -> RealtimeHandshake:
    ...
    def note_session_payload(self, session_payload: Mapping[str, Any]) -> None:
    ...
    def is_envelope_event(self, payload: Mapping[str, Any]) -> bool:
        return payload.get("type") in ENVELOPE_EVENT_TYPES

    def translate(self, payload: Mapping[str, Any]) -> DuplexCommand:
