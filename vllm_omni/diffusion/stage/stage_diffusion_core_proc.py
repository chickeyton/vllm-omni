...
``StageDiffusionCoreProc`` runs ``DiffusionEngine`` in a child process,
communicating with ``StageDiffusionCoreClient`` via ZMQ (PUSH/PULL). Request and
result frames are typed ``StageDiffusion*`` msgspec structs
(``stage_core_types``); control frames (abort / collective_rpc / shutdown) remain
lightweight ``{"type": ...}`` maps, distinguished on the wire by the presence of
a ``"type"`` key.
"""
...
class StageDiffusionCoreProc:
    ...
    async def run_loop(
        ...
        """Async event loop handling ZMQ messages from StageDiffusionCoreClient."""
        ...
        def _encode_result(result: OmniRequestOutput, request_id: str) -> bytes:
            """Wrap one diffusion result in the typed output envelope."""
        ...
        async def _dispatch_request(
            ...
            try:
                if not self._od_config.streaming_output:
                    ...
                    await response_socket.send(_encode_result(result, request_id))
                else:
        ...
        try:
            while True:
                ...
                # Data frames are ``StageDiffusionCoreRequest`` structs (decoded
                # as a map with no ``"type"`` key); control frames keep the
                # lightweight ``{"type": ...}`` shape.
                if not (isinstance(msg, dict) and "type" in msg):
                    request = msgspec.convert(msg, StageDiffusionCoreRequest)
            ...
            try:
                ...
                await response_socket.send(StageDiffusionCoreProc.DIFFUSION_PROC_DEAD)
            except Exception:
    ...
    def run_diffusion_proc(
        ...
        *,
        ...
        try:
            ...
            logger.debug("StageDiffusionCoreProc exiting.")
            raise
        except Exception:
            logger.exception("StageDiffusionCoreProc encountered a fatal error.")
            raise
        finally:
