...
def _load_tokenizer(model_config: Any) -> Any | None:
...
def _stage0_stop_token_ids(tokenizer: Any | None) -> list[int]:
...
def _scheduler_token_id(tokenizer: Any | None) -> int | None:
    if tokenizer is None:
        return None
    scheduler_tokens = (
...
class MiniCPMO45DuplexPlugin(DuplexModelPlugin):
    ...
    def __init__(self, encode_audio: EncodeAudio) -> None:
        ...
        self._tokenizers: dict[str, Any | None] = {}
        self._tokenizer_lock = asyncio.Lock()

    async def _tokenizer_for(self, model_config: Any) -> Any | None:
        model_path = getattr(model_config, "model", None)
    ...
    async def prepare_runtime_config(self, config: DuplexSessionConfig, *, model_config: Any) -> dict[str, object]:
        ...
        if any(key in extra_body for key in ("ref_audio_path", "tts_ref_audio_path")):
            raise ValueError("ref_audio_path is not accepted by duplex sessions; use ref_audio URI instead")
