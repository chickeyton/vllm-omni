...
_TRANSPORT_ONLY_EVENTS = frozenset({"session.heartbeat", "session.event_ack"})
...
class InlineDuplexClient(DuplexClientBase):
    ...
    async def _open(self) -> None:
        ...
        self._handle = await self.omni.open_session(
            self.config.to_session_payload(model=self.model, session_id=self.session_id),
    ...
    async def _send_command(self, payload: dict[str, object]) -> None:
        ...
        try:
            command = command_from_realtime(payload)
