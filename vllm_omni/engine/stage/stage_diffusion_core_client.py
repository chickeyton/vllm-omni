...
Owns the frontend-side ZMQ sockets for StageDiffusionCoreProc and exposes the
...
def create_diffusion_client(
    ...
    return StageDiffusionCoreClient.from_addresses(
        metadata,
...
class StageDiffusionCoreClient(StageCoreClientBase):
    ...
    def _initialize_client(
        ...
        *,
        ...
        logger.info(
            "[StageDiffusionCoreClient] stage-%s [rep-%s] initialized (owns_process=%s, batch_size=%d)",
            self.stage_id,
            self.replica_id,
    ...
    def _start_proc_monitor(self) -> None:
        ...
        def _monitor() -> None:
            ...
            logger.error(
                "[StageDiffusionCoreClient] stage-%s [rep-%s] StageDiffusionCoreProc died unexpectedly (exit code %s).",
                client.stage_id,
                client.replica_id,
                proc.exitcode,
            )
    ...
    def _drain_responses(self) -> None:
        ...
        while True:
            ...
            # Data frames are ``StageDiffusionCoreOutputs`` structs (decoded as a
            # map with no ``"type"`` key); control frames keep ``{"type": ...}``.
            ...
            if msg_type == "rpc_result":
                ...
                logger.error(
                    "[StageDiffusionCoreClient] stage-%s [rep-%s] subprocess error for %s: %s",
                    self.stage_id,
                    self.replica_id,
                    rpc_id or req_id,
                    error_msg,
    ...
    def _engine_dead_reason(self) -> str | None:
        ...
        Also caches a newly-detected death in ``_engine_dead`` as a side effect.
        """
        if self._engine_dead:
            return f"Stage-{self.stage_id} diffusion subprocess is dead"
        if self._proc_manager is None:
            return None
        proc = self._proc_manager.proc
