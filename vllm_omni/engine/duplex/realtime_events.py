...
class RealtimeProjectionState:
    ...
    def apply_session_defaults(self, session_payload: Mapping[str, Any]) -> None:
...
def _remember_response_audio_metadata(
    state: RealtimeProjectionState, response_id: object, event: Mapping[str, Any]
) -> None:
    projection = _response_state(state, response_id)
    if projection is None:
        return
...
def _response_created_event(event: Mapping[str, Any]) -> ResponseCreated:
...
def _response_speak_metadata(event: Mapping[str, Any]) -> dict[str, object]:
...
def _realtime_audio_done_events(
    state: RealtimeProjectionState, event: Mapping[str, Any], response_id: object
) -> list[DuplexEvent]:
    item_id = _response_item_id(state, response_id)
...
def _input_audio_transcription_completed_event(
    item_id: str, item: Mapping[str, Any]
...
def realtime_session_payload(state: RealtimeProjectionState, session: object) -> dict[str, Any]:
...
def project_internal_event(state: RealtimeProjectionState, event: Mapping[str, Any]) -> list[DuplexEvent]:
...
def _project(state: RealtimeProjectionState, event: dict[str, Any]) -> list[DuplexEvent]:
        ...
        events: list[DuplexEvent] = [
            SessionCreated(
                ...
                attachment_generation=(
                    event["attachment_generation"] if isinstance(event.get("attachment_generation"), int) else None
                ),
    ...
    if event_type == "response.output_audio.delta":
        ...
        if has_text:
            _append_response_transcript(state, response_id, text)
            _refresh_in_progress_response_item(state, response_id)
...
def _function_call_done_events(state: RealtimeProjectionState, event: dict[str, Any]) -> list[DuplexEvent]:
...
def retrieve_item_events(state: RealtimeProjectionState, payload: Mapping[str, Any]) -> list[DuplexEvent]:
...
def _remember_input_transcript_hint(state: RealtimeProjectionState, event: Mapping[str, Any]) -> None:
...
def resolve_cancel_response(state: RealtimeProjectionState, command: CancelResponse) -> ResolvedControl:
    payload: dict[str, Any] = {"type": "response.cancel", "reason": "response.cancel"}
...
def resolve_clear_output_audio(state: RealtimeProjectionState, command: ClearOutputAudio) -> ResolvedControl:
    payload: dict[str, Any] = {"type": "output_audio_buffer.clear", "reason": "output_audio_buffer.clear"}
    response_id = command.response_id or state.active_response_id or state.last_response_id
    if response_is_done(state, response_id):
...
def append_audio_payload(command: AppendAudio) -> dict[str, Any]:
