...
_IDENTITY_FIELDS = frozenset({"session_id", "incarnation", "epoch", "event_id"})
...
class SessionCreated(DuplexEvent):
    ...
    def _wire_fields(self) -> dict[str, Any]:
        return {"incarnation": self.incarnation, **DuplexEvent._wire_fields(self)}
...
class SessionResumed(SessionCreated):
    ...
    def _wire_fields(self) -> dict[str, Any]:
        return {"session_id": self.session_id, **SessionCreated._wire_fields(self)}
...
@dataclass(frozen=True, slots=True, kw_only=True)
class RuntimeControl(DuplexEvent):
    """Redacted runtime-control notice from the model plugin."""

    wire_type = "runtime.control"

    details: Mapping[str, Any] = field(default_factory=dict)

    def _wire_fields(self) -> dict[str, Any]:
        return {k: _wire_value(v) for k, v in self.details.items() if k != "type"}
...
class Listen(DuplexEvent):
    ...
    def _wire_fields(self) -> dict[str, Any]:
        ...
        data: dict[str, Any] = {"session_id": self.session_id, "epoch": self.epoch, "response": response}
        if self.response_id:
...
class ContentPartAdded(_ResponseEvent):
    ...
    part: Mapping[str, Any] = field(default_factory=dict)
...
class RateLimitsUpdated(DuplexEvent):
    ...
    rate_limits: tuple[Mapping[str, Any], ...] = ()
...
class ErrorEvent(DuplexEvent):
    ...
    param: str | None = None
    #: Extra keys merged into the wire ``error`` object (``retryable``, ``runtime_control`` ...).
    ...
    def error(self) -> dict[str, Any]:
        error: dict[str, Any] = {"type": self.error_type, "code": self.code, "message": self.message}
        if self.related_event_id:
            error["event_id"] = self.related_event_id
        if self.param:
            error["param"] = self.param
        error.update(_wire_value(self.extra))
        return error
