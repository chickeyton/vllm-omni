...
``DuplexOrchestrator`` hosts exactly one manager. It replaces the former
``DuplexControlPlane`` + ``DuplexSessionRuntimeManager`` pair and the
serving-side ``DuplexSessionRegistry`` / lifecycle listener: the whole session
(``DuplexEngineSession``) lives here, owned by one ``DuplexSessionRunner`` per
session on the orchestrator loop.
...
from vllm_omni.engine.duplex.events import DuplexEvent, ErrorEvent, error_event
...
class DuplexSessionManager:
    ...
    def dispatch(self, message: object) -> None:
        ...
        task = asyncio.create_task(run_ordered(), name=f"duplex-control-{session_id}-{message.control_id}")
        ...
        def discard(completed: asyncio.Task[None]) -> None:
            ...
            if not completed.cancelled() and completed.exception() is not None:
                logger.error("duplex control task failed: %r", completed.exception())
    ...
    def _dispatch_command(self, message: DuplexSessionCommandMessage) -> None:
        ...
        admission holds one pending-turn reservation which the runner releases
        when it dequeues the command (as the old read loop / ``next_actor_event``
        pair did). Input bytes are only *checked* here against the session limit;
        the runner performs the real byte reservation and release around its PCM
        reservation exactly as before.
        """
        runner = self.runners.get(message.session_id)
        command = message.command
        if runner is None or runner.session.state == DuplexSessionState.CLOSED:
            self._emit_raw(
                message.session_id,
                message.incarnation,
                self._error_event(
        ...
        if session.incarnation != message.incarnation:
            self._emit_raw(
                message.session_id,
                message.incarnation,
                self._error_event(
                    "stale_incarnation",
                    f"Duplex session {message.session_id} incarnation {message.incarnation} is stale",
                    command=command,
                ),
            )
            return
        if isinstance(command, AppendAudio):
            size = len(command.audio)
            limit = int(self.runtime_config.max_pending_input_bytes_per_session)
            if session.pending_input_bytes + size > limit:
                self.emit(
                    session,
                    self._error_event(
                        "input_backpressure",
    ...
    def emit(self, session: DuplexEngineSession, event: DuplexEvent) -> None:
        ...
        self._emit_raw(session.session_id, session.incarnation, event, epoch=session.epoch)
    ...
    def _emit_raw(
        ...
        *,
        ...
        event = replace(event, session_id=session_id, incarnation=incarnation, epoch=epoch)
        message = DuplexSessionEventMessage(session_id=session_id, incarnation=incarnation, event=event)
    ...
    def _admission_count(self) -> int:
        return len(set(self.runners) | set(self._closing))
    ...
    async def open(self, message: OpenDuplexSessionMessage) -> None:
        ...
        try:
            if session_id in self.runners or session_id in self._closing:
            ...
            self._next_incarnation_by_session_id[session_id] = incarnation + 1
    ...
    def _require_runner(self, session_id: str, incarnation: int | None = None) -> DuplexSessionRunner:
        runner = self.runners.get(session_id)
        if runner is None:
            raise DuplexSessionError(f"Unknown duplex session: {session_id}", code="not_found")
        if incarnation is not None and runner.session.incarnation != incarnation:
            raise DuplexSessionError(
                f"Duplex session {session_id} incarnation {incarnation} is stale",
    ...
    async def close(self, message: CloseDuplexSessionMessage) -> None:
        ...
        try:
            ...
            await self._put_result(message, operation="close", ok=False, session=session, error=exc)
    ...
    def close_sessions_for_request_ids(
        ...
        *,
        ...
        for session_id, runner in list(self.runners.items()):
            ...
            self._dispatched_control_tasks.add(task)
            task.add_done_callback(self._dispatched_control_tasks.discard)
        return closed
    ...
    async def _run_request_cleanup(self, key: tuple[str, int, int], pending: _PendingRequestCleanup) -> None:
