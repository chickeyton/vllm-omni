...
class RealtimeInputDefaults:
    ...
    def with_session_payload(self, session_payload: Mapping[str, Any]) -> RealtimeInputDefaults:
...
def validate_realtime_session_audio_formats(session_payload: Mapping[str, Any]) -> str | None:
...
def validate_realtime_response_audio_formats(response_payload: Mapping[str, Any]) -> str | None:
...
def realtime_overlap_fields(session_payload: Mapping[str, Any]) -> dict[str, object]:
...
def json_safe_realtime_payload(payload: Mapping[str, Any]) -> dict[str, object]:
...
def input_explicitly_non_speech(event: Mapping[str, Any]) -> bool:
...
def copy_realtime_input_hints(source: Mapping[str, Any], target: dict[str, object]) -> None:
...
def normalize_conversation_item(item: Mapping[str, Any]) -> dict[str, object]:
...
def validate_realtime_item_truncate(item: Mapping[str, Any], *, content_index: int, audio_end_ms: int) -> str | None:
...
def input_transcript_from_item(item: Mapping[str, Any]) -> str:
    content = item.get("content")
    if not isinstance(content, list):
        return ""
    parts: list[str] = []
...
def build_append_audio(
    ...
    *,
    defaults: RealtimeInputDefaults,
    hints_source: Mapping[str, Any] | None = None,
) -> AppendAudio:
...
def translate_realtime_command(
    ...
    *,
    ...
    if event_type == "session.update":
        ...
        session_payload: Mapping[str, Any] = session if isinstance(session, dict) else payload
    ...
    if event_type in {"input_audio_buffer.append", "input.audio.append", "push_chunk"}:
        event = dict(payload)
        if event_type != "input_audio_buffer.append":
            event.setdefault("format", "wav")
        return build_append_audio(event, defaults=defaults)
