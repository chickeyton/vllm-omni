...
class DuplexSessionConfig:
    ...
    def from_realtime(
        ...
        *,
        ...
        if isinstance(payload.get("turn_detection"), dict):
            extra_body["realtime_turn_detection"] = dict(payload["turn_detection"])
        ...
        return cls.from_event({"session": session, "session_id": session_id or payload.get("session_id")})
    ...
    def apply_realtime_update(
        ...
        *,
        ...
        Returns ``None`` on success or the internal ``error`` event dictionary
        (``code`` in ``model_update_unsupported``, ``voice_update_after_audio_unsupported``,
