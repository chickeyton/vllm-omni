...
This module is the moved body of the former serving-side
``DuplexSessionRunnerMixin.handle_session`` closure, ``NativeRuntimeBridgeMixin``
and the domain helpers of ``OmniDuplexSessionHandler``, plus the append /
cancel algorithms of the former ``DuplexControlPlane``. It runs on the
orchestrator loop:

* every mutation of ``DuplexEngineSession`` happens on this loop, through the
  mailbox worker (commands, stage outputs, timers, internal items) or through
  tracked tasks that re-validate ``(incarnation, epoch, turn_id)`` after each
  ``await``;
* appends are planned with the model plugin and submitted to the stage port
  in wire order on the per-session append tail, exactly like the old
  ``start_native_append`` chain, but without any RPC hop;
* stage outputs are pushed in by ``DuplexOrchestrator._intercept_stage_output``
  instead of being polled through request queues;
* everything the session says leaves as Realtime wire payloads through
  ``DuplexSessionManager.emit`` after the same terminal-acceptance / stale
  filtering the old writer applied.
"""
...
# --------------------------------------------------------------------------- #
# Task bookkeeping (moved from entrypoints/duplex/websocket.py)               #
# --------------------------------------------------------------------------- #
...
class DuplexSessionTasks:
    """Connection-independent task handles for one session."""
...
@dataclass(frozen=True, slots=True)
class _Internal:
    """Runner-internal mailbox item (payload runs through the legacy dict handlers)."""
...
class DuplexSessionRunner:
    ...
    def start(self) -> None:
        ...
        self.model_state.silence_continuation_scheduler = self._schedule_silence_continuation
    ...
    def on_stage_failure(self, stage_id: int, exc: BaseException) -> None:
        self._mailbox.put_nowait(_StageFailure(stage_id=stage_id, exc=exc))

    async def close(self, reason: str) -> None:
        """Graceful close: cancel work, release the data plane, emit ``session.closed``."""
        session = self.session
        if session.state == DuplexSessionState.CLOSED and self._closed_emitted:
    ...
    async def expire(self, reason: str) -> None:
        session = self.session
        self._begin_close(reason)
        if not self._closed_emitted:
            self._closed_emitted = True
    ...
    async def _handle_item(self, item: DuplexCommand | _StageOutput | _StageFailure | _Internal) -> None:
        session = self.session
        if isinstance(item, _StageOutput):
            await self._on_stage_output_item(item)
            return
        if isinstance(item, _StageFailure):
            await self._on_stage_failure_item(item)
            return
    ...
    async def _run_internal_payload(self, payload: dict[str, Any]) -> None:
        """Run one legacy internal event dictionary through the matching handler."""
    ...
    async def _on_close_command(self, reason: str) -> None:
        ...
        # NOTE(refactor): a close requested through the command stream (not the
    ...
    def _is_stale_model_output(self, payload: dict[str, object]) -> bool:
        ...
        if event_type not in MODEL_OUTPUT_EVENTS:
            return False
        if self._closing and event_type not in {"response.listen", "runtime.control"}:
            return True
        if self.session.state == DuplexSessionState.CLOSED and event_type not in {"response.listen", "runtime.control"}:
            return True
    ...
    def _stage0_request_id(self, epoch: int) -> str:
        ...
        return duplex_resource_request_id(
            DuplexFence(session.session_id, epoch=epoch, incarnation=session.incarnation),
            "stage0",
        )
    ...
    async def _run_turn_detection(self, event: dict[str, Any]) -> TurnDetectionResult | None:
    ...
    async def _on_append_audio(self, event: dict[str, Any]) -> None:
        session = self.session
        model_state = self.model_state
        session.mark_user_input_activity()
        ...
        if overlap_active:
            ...
            if action == "listen":
                ...
                had_stream = await self._cancel_data_plane_stream() or had_stream
    ...
    # ------------------------------------------------------------------ #
    # Engine bridge (was NativeRuntimeBridgeMixin + control plane append) #
    # ------------------------------------------------------------------ #
        ...
        *,
        ...
        try:
            ...
            try:
                ...
                if session.incarnation != fence.incarnation or (
    ...
    async def _cancel_data_plane_stream(self) -> bool:
        had_stream = self._stream_request_id is not None
        self._stream_request_id = None
        self.model_state.data_plane_task = None
        self.model_state.data_plane_restart_requested = False
        return had_stream
    ...
    def _decide_output(self, stage_id: int, output: Any, context: DuplexOutputContext) -> DuplexOutputDecision | None:
    ...
    async def _on_stage_output_item(self, item: _StageOutput) -> None:
        ...
        if finished and emitted_response and not self._session_auto_responds():
            # The old drain task returned here and, on its way out, released
            # the per-request projector cursor and offered the model another
            # silence unit.
    ...
    # ------------------------------------------------------------------ #
    # Silence continuation (was NativeRuntimeBridgeMixin)                #
    # ------------------------------------------------------------------ #

    def _silence_unit_payload(self) -> dict[str, object]:
        samples = int(getattr(self.plugin, "silence_continuation_samples", 16000))
        audio = (
    ...
    async def _maybe_continue_response(
        ...
        *,
        ...
        scheduler = model_state.silence_continuation_scheduler
    ...
    async def _on_cancel(self, event: dict[str, Any]) -> None:
        session = self.session
        model_state = self.model_state
    ...
    async def _on_turn_signal(self, event: dict[str, Any]) -> None:
        session = self.session
    ...
    async def _on_session_update(self, payload: dict[str, Any], *, realtime_event_id: str | None) -> None:
        ...
        update_error = candidate_config.apply_realtime_update(
            payload,
            session_id=session.session_id,
    ...
    async def _on_commit(self, event: dict[str, Any]) -> None:
        session = self.session
        model_state = self.model_state
        event_type = str(event.get("type"))
    ...
    def _handle_playback_ack(self, event: dict[str, Any]) -> None:
        session = self.session
