...
class Commit(DuplexCommand):
    ...
    final: bool = True
    #: ``None`` keeps the session policy (auto-response); ``True``/``False`` force it.
...
def command_from_realtime(payload: Mapping[str, Any]) -> DuplexCommand:
    """Translate one OpenAI Realtime client event into a command.

    Envelope concerns (``event_id`` acknowledgements, ``conversation.item.retrieve``,
    resume/heartbeat transport bookkeeping) are handled by the caller; this
...
def commands_from_sequence(payloads: Sequence[Mapping[str, Any]]) -> list[DuplexCommand]:
    return [command_from_realtime(p) for p in payloads]
